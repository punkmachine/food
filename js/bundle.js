...
function modal(triggerSelector, modalSelector) {
  ...
  function bindPostData(form) {
    form.addEventListener('submit', function (event) {
      ...
      statusMessage.style.cssText = `
				...
				margin: 10 auto;
			`;
      form.insertAdjacentElement('afterend', statusMessage); //конструкция данных из форм

      const formData = new FormData(form); //Превращение данных в матрицу, потом в объект.

      let json = Object.fromEntries(formData.entries()); //обработка для заглавной буквы

      json.name = S(`${json.name}`).capitalize().s; //превращение данных в json

      json = JSON.stringify(json); //обработка промиса

      (0,_services_services__WEBPACK_IMPORTED_MODULE_0__.postData)('http://localhost:3000/requests', json).then(function (data) {
  ...
  window.addEventListener('scroll', showModalByScroll); //привязка функций к каждой форме
...
function slider({
  ...
  const indicators = document.createElement('ol'); //массив с точками в тавигации слайдера
  ...
  function arrowOrDotsClick() {
    ...
    currentSlide.innerHTML = (0,_services_services__WEBPACK_IMPORTED_MODULE_0__.getZero)(countSlide); //удаление класса активности у всех точек
  ...
  slider.style.position = 'relative'; //индикаторы слайдера

  indicators.classList.add('carousel-indicators');
  slider.append(indicators); //создание точек индикаторов
...
/* harmony export */   "getZero": function() { return /* reexport safe */ _services_services__WEBPACK_IMPORTED_MODULE_0__.getZero; }
/* harmony export */ });
/* harmony import */ var _services_services__WEBPACK_IMPORTED_MODULE_0__ = __webpack_require__(/*! ../services/services */ "./js/services/services.js");
...
function timer() {
  ...
  function setClock(selector, endtime) {
    ...
    function updateClock() {
      ...
      seconds.innerHTML = (0,_services_services__WEBPACK_IMPORTED_MODULE_0__.getZero)(t.seconds); //остановка таймера.
